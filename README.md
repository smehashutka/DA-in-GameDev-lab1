...
Импортируем необходиые библиотеки, упаковываем обучающие данные в массивы.
...
#выводим график с точками на координатной плоскости
plt.scatter(x,y)
...
# возвращает значение функции, то есть предсказанное значение y
def model(a, b, x):
  return a*x + b

#вычисляет среднеквадратичную ошибку (значение переменной loss) - то есть ищет среднее ошибок для каждого элемента из массива
def loss_function(a, b, x, y):
  num = len(x)
  prediction=model(a,b,x)
  return (0.5/num) * (np.square(prediction-y)).sum()

# подбирает веса а и b, здесь реализуется метод градиентного спуска - частные производные da и db показывают оптимальное "направление", 
#а величина Lr (Learning rate) - коэффициент скорости (шага) обучения
...
#задает количество итераций подбора коэффициентов
...
#рандомно задаем коэффициенты a и b в промежутке от [0,1), а также в ручную переменную Lr
a = np.random.rand(1)
print(a)
b = np.random.rand(1)
print(b)
...
#в ручную есть возможность задавать количество итераций
a,b = iterate(a,b,x,y,9000000)
prediction=model(a,b,x)
